...
import { Heart, ShoppingCart, Star, ChevronLeft, ChevronRight, Minus, Plus, Search, Truck, Rocket, Zap, RotateCcw, Package, CreditCard, ThumbsUp, Instagram, Youtube } from "lucide-react";
...
const ProductDetail: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Navigation */}
      <nav className="bg-muted py-2 text-sm">
        <div className="container mx-auto px-4">
          <ul className="flex space-x-2">
            ...
            <li>
              <span className="text-foreground">iMac</span>
            </li>
          </ul>
        </div>
      </nav>
              ...
              <div className="flex">
                ...
                <div className="w-20 mr-4 space-y-3">
                  {productImages.map((image, index) => (
                    <div
                      key={index}
                      className={`border rounded cursor-pointer overflow-hidden ${selectedImage === index ? "border-primary" : "border-border"}`}
                ...
                <div className="flex-1 relative border border-border rounded overflow-hidden">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="absolute top-4 right-4 bg-background rounded-full shadow-md"
                    onClick={toggleWishlist}
                  >
                    <Heart className={`h-5 w-5 ${isWishlisted ? "text-red-500 fill-red-500" : "text-muted-foreground"}`} />
                  </Button>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex items-center mt-2">
                    ...
                    <span className="text-sm text-muted-foreground ml-2">4.7 (1,283 reviews)</span>
                  </div>
                  <div className="mt-2 flex items-center gap-2">
                    <Badge variant="secondary" className="bg-green-100 text-green-800">
                      In Stock
                    </Badge>
                    <span className="text-sm text-muted-foreground">• Released on June 2025</span>
                  </div>
                </div>

                <div className="border-t border-b border-border py-4">
                  <div className="flex items-baseline">
                    <span className="text-3xl font-bold text-foreground">62,910,000 đ</span>
                    <span className="text-lg text-muted-foreground line-through ml-2">$69,900,000 đ</span>
                    <Badge variant="secondary" className="ml-3 bg-orange-100 text-orange-800 text-xl">
                      10% off
                    </Badge>
                  </div>
                  <p className="text-sm text-muted-foreground mt-2">
                    Free shipping by Jun 23, 2025 • Eligible for Return, Refund or Replacement within 30 days of receipt if
                    you are not fully satisfied (within 14 days for Outlet).{" "}
        ...
        <div className="container mx-auto px-4">
          <Tabs defaultValue="description" className="w-full">
            <TabsList className="grid w-full grid-cols-4 mb-8">
              ...
              <TabsTrigger value="reviews">Ratings & reviews (1,283)</TabsTrigger>
            </TabsList>

            <TabsContent value="description" className="bg-background p-6 rounded-lg shadow-sm">
              <div className="space-y-4">
                <p className="text-muted-foreground">
                  <span className="font-bold">LIGHTNING FAST.</span> The M1 chip delivers speed to tackle any all-day
                  workflow, with a big leap in CPU, GPU, and machine learning performance. The iMac 24-inch delivers
                  impressive performance in an incredibly thin design. The new iMac features a 24-inch 4.5K Retina display
                  with 11.3 million pixels, 500 nits of brightness, and over a billion colors.
                </p>
                <p className="text-muted-foreground">
                  It has a 1080p FaceTime camera, studio-quality mics, and a six-speaker sound system — the best camera and
                  audio ever in a Mac. Touch ID comes to iMac, making it easier than ever to securely log in, make
                  purchases, switch user profiles, and more. The new iMac joins the family of Mac models powered by M1,
                  including MacBook Air, 13-inch MacBook Pro, and Mac mini, marking another step forward in Apple's
                  transition to Apple silicon.
                </p>
                <p className="text-muted-foreground">
                  Combining the power of M1 with macOS Big Sur, the new iMac instantly wakes from sleep, everyday tasks feel
                  noticeably faster and fluid, and apps launch with amazing speed. The 8-core CPU delivers up to 85 percent
                  faster performance than the previous generation, while the 7-core or 8-core GPU delivers up to 2x faster
                ...
                <p className="text-muted-foreground">
                  <span className="font-bold">SIMPLY STUNNING.</span> The new iMac features a 24-inch 4.5K Retina display
                  with narrow borders and 11.3 million pixels. With a P3 wide color gamut and over a billion colors, plus
                  500 nits of brightness, images on the new iMac are vivid and more brilliant. This spectacular display
                  features True Tone technology, which automatically adjusts the color temperature of the display to match a
                  user's ambient lighting.
                </p>
                <p className="text-muted-foreground">
                  The combination of the outstanding Retina display, powerful speakers, and spacious microphones
                  makes the ideal machine for all your needs. The new iMac has the best sound system ever in a Mac. It
                  features a six-speaker sound system, with force-backed woofers placed back-to-back for an impressive
                  response, while reducing unwanted vibrations.
                </p>
              </div>
            </TabsContent>

            <TabsContent value="specifications" className="bg-background p-6 rounded-lg shadow-sm">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="font-bold text-foreground mb-3">Technical Specifications</h3>
                  <ul className="space-y-2">
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Processor</span>
                      <span className="w-2/3 text-foreground">Apple M1 chip with 8-core CPU</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Memory</span>
                      <span className="w-2/3 text-foreground">8GB unified memory</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Storage</span>
                      <span className="w-2/3 text-foreground">256GB SSD</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Graphics</span>
                      <span className="w-2/3 text-foreground">7-core GPU</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Display</span>
                      <span className="w-2/3 text-foreground">24-inch 4.5K Retina display (4480 x 2520)</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Camera</span>
                      <span className="w-2/3 text-foreground">1080p FaceTime HD camera</span>
                    </li>
                  </ul>
                </div>
                <div>
                  <h3 className="font-bold text-foreground mb-3">Connectivity</h3>
                  <ul className="space-y-2">
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Wi-Fi</span>
                      <span className="w-2/3 text-foreground">Wi-Fi 6 (802.11ax)</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Bluetooth</span>
                      <span className="w-2/3 text-foreground">Bluetooth 5.0</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Ports</span>
                      <span className="w-2/3 text-foreground">Two Thunderbolt / USB 4 ports, Two USB 3 ports</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Audio</span>
                      <span className="w-2/3 text-foreground">Six-speaker system with force-cancelling woofers</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Microphone</span>
                      <span className="w-2/3 text-foreground">Studio-quality three-mic array</span>
                    </li>
                    <li className="flex">
                      <span className="w-1/3 text-muted-foreground">Power</span>
                      <span className="w-2/3 text-foreground">143W power adapter</span>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <TabsContent value="reviews" className="bg-background p-6 rounded-lg shadow-sm">
              <div className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                  <div className="flex items-center">
                    <div className="mr-4">
                      ...
                      <div className="text-sm text-muted-foreground mt-1">1,283 reviews</div>
                    </div>
                      ...
                      <div className="space-y-1">
                        {[
                          ...
                          <div key={rating.stars} className="flex items-center">
                            ...
                            <span className="text-xs w-8 text-right">{rating.percentage}%</span>
                          </div>
                        ))}
                      </div>
                    </div>
                        ...
                        <div className="flex justify-between">
                          <div>
                            ...
                            <div className="flex items-center mt-1">
                              ...
                              <span className="text-sm text-muted-foreground ml-2">Verified Purchase</span>
                            </div>
                          </div>
            ...
            {similarProducts.map((product) => (
              ...
              >
                ...
                <CardContent className="p-4">
                  <h3 className="font-medium text-foreground text-sm line-clamp-2 h-10">{product.name}</h3>
                  <div className="flex items-center mt-2">
                    <div className="flex text-yellow-400 text-xs">
                      {[...Array(Math.floor(product.rating))].map((_, i) => (
                        <Star key={i} className="h-3 w-3 fill-current" />
                      ))}
                      {product.rating % 1 !== 0 && <Star className="h-3 w-3 fill-current" />}
                    </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
            <div>
              ...
              <ul className="space-y-2">
                {["About Us", "Careers", "Corporate Responsibility", "Press Center", "Investor Relations"].map(
                  (item) => (
                    <li key={item}>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {[
                  { name: "Facebook", icon: <Facebook className="text-blue-600 mr-2 h-4 w-4" /> },
                  { name: "Instagram", icon: <Instagram className="text-pink-600 mr-2 h-4 w-4" /> },
                  { name: "YouTube", icon: <Youtube className="text-red-600 mr-2 h-4 w-4" /> },
                ].map((social) => (
                  <li key={social.name}>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {["Help Center", "Track Your Order", "Returns & Exchanges", "Shipping Information", "Contact Us"].map(
                  (item) => (
                    <li key={item}>
                      <a href="#" className="text-sm text-muted-foreground hover:text-primary">
                        {item}
            ...
            <div>
              ...
              <div className="mt-6">
                <h3 className="font-bold text-foreground mb-4">Subscribe to our Newsletter</h3>
                <div className="flex">
                  <Input
                    type="email"
